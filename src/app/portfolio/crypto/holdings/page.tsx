...
import { fmtDay, fmtPct, fmtMoney, tone } from "@/lib/portfolio/format";
...
export default async function CryptoHoldings() {
    ...
    <div className="flex flex-col gap-5">
      ...
      <Card>
        ...
        <div className="mb-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">
          No long-term/short-term tax classification is shown here: India taxes crypto gains at a flat
          30% under Section 115BBH regardless of holding period (no LTCG/STCG distinction, no loss
          set-off against other income), so a tax clock like the US/foreign-equity screens' would be
          misleading here — to be confirmed with your CA before any tax figure is added.
        </div>
      ...
      <Card className="overflow-x-auto p-0">
        ...
        {disposals.length === 0 ? (
          ...
          <table className={tableCls}>
            <thead className={theadCls}>
              <tr><Th right={false}>Coin</Th><Th right={false}>Opened</Th><Th right={false}>Sold</Th><Th>Units</Th><Th>Cost</Th><Th>Proceeds</Th><Th>Gain</Th><Th>Held</Th></tr>
            </thead>
            <tbody>
              {disposals.map((d, i) => (
        ...
        <div className="px-5 pb-4 pt-2"><Note>FIFO, fees folded into cost and proceeds, INR. No sales recorded yet in the CoinDCX order history — all 8 orders are buys.</Note></div>
      </Card>
    </div>
  );
}
