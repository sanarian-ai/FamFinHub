...
export function HolderToggle({ base, q }: { base: string; q: Q }) {
  return (
    <Pills>
      {HOLDER_ORDER.map((k) => (
        <Pill key={k} to={href(base, q, { h: k })} active={(q.h ?? "HOUSEHOLD") === k}>
          {HOLDER_LABEL[k]}
        </Pill>
      ))}
    </Pills>
...
export function OpenClosedToggle({ base, q }: { base: string; q: Q }) {
  return (
    <Pills>
      {(["ALL", "OPEN", "CLOSED"] as OpenClosed[]).map((k) => (
        <Pill key={k} to={href(base, q, { oc: k })} active={(q.oc ?? "ALL") === k}>
          {OC_LABEL[k]}
        </Pill>
      ))}
    </Pills>
