 ...
 * to INR and merges both books' cash-flow streams (see combined.ts's header for the Modified-Dietz
 * merge fix, 2026-09-26). Real estate has no engine-backed IRR at all and stays off this page
 * entirely — see Overview. Crypto (BTC + ETH) joined as a 3rd book on 2026-09-26 (see combined.ts's
 * cryptoCtx param): it folds into every All Assets total (value, P&L, the merged IRR) and gets its
 * own row in "By book" below, but its own alpha vs Nifty 50 TRI stays on its own Performance tab
 * rather than being blended into the tiles here.
 *
 * Benchmark tiles/table/chart show `main.allBench` — the WHOLE household's merged cash flows
 * replicated into each of the 3 locked indices (Nifty 50 TRI / S&P 500 / QQQ), i.e. "if every
 * rupee/dollar actually invested had instead gone into this index." Alpha is All Assets IRR minus
 * that same-scope replica — changed 2026-09-26 from the prior book-scoped replica (India's flows
 * into Nifty, US's flows into SPY/QQQ), which mixed a whole-portfolio return against a single-book
 * benchmark and didn't answer a coherent counterfactual. See combined.ts's header for the full
 * before/after.
 *
...
export default async function AllPerformance({ searchParams }: { searchParams: Promise<{ [k: string]: string | string[] | undefined }> }) {
  ...
  const niftyLeg = main.allBench[COMBINED_INDIA_BENCHMARK];
  const spLeg = main.allBench.SPY, qqqLeg = main.allBench.QQQ;
  ...
  // Merged value series: both run() calls inside combinedRun() were given the same start/end and
  // identical Ctx.dates arrays (see combined.ts's header comment), so the US and India series are
  // index-aligned date-for-date — no need to re-key by date string. Benchmark lines come from
  // main.allBenchSeries — the whole household's merged flows replicated into each index — not each
  // book's own series, so they represent the same counterfactual as the tiles/table above.
  const mergedSeries: SeriesPt[] = (main.us.series ?? []).map((u, i) => {
    const ind = main.india.series?.[i];
    const cr = main.crypto?.series?.[i];
    const bs = main.allBenchSeries!;
    const bench = { [COMBINED_INDIA_BENCHMARK]: bs[COMBINED_INDIA_BENCHMARK][i], SPY: bs.SPY[i], QQQ: bs.QQQ[i] };
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        ...
        <Tile
          ...
          sub={<>whole portfolio&apos;s flows replicated · <span className={tone(alphaNifty)}>α {fmtPP(alphaNifty)}</span></>}
        />
        <Tile
          label={`${BENCHMARK_LABEL.SPY} ${kind}`}
          dot={CATEGORICAL[2]}
          value={spH.value == null ? "n/a" : fmtPct(spH.value)}
          sub={<>whole portfolio&apos;s flows replicated · <span className={tone(alphaSP)}>α {fmtPP(alphaSP)}</span></>}
        />
        <Tile
          label={`${BENCHMARK_LABEL.QQQ} ${kind}`}
          dot={CATEGORICAL[3]}
          value={qqqH.value == null ? "n/a" : fmtPct(qqqH.value)}
          sub={<>whole portfolio&apos;s flows replicated · <span className={tone(alphaQQQ)}>α {fmtPP(alphaQQQ)}</span></>}
        />
      </div>

      <Card>
        <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="text-sm font-semibold text-slate-900">{period.label}: All Assets value vs the whole household&apos;s flows in each index</h2>
        ...
        <Note>
          Replica lines replicate the WHOLE household&apos;s merged cash flows into each index — every rupee/dollar actually invested, on the
        ...
        <table className={tableCls}>
          ...
          <tbody>
            {table.map(({ d, r }) => {
              ...
              const niftyH = headline(r.allBench[COMBINED_INDIA_BENCHMARK], r);
              const spH = headline(r.allBench.SPY, r);
              const qqqH = headline(r.allBench.QQQ, r);
              return (
