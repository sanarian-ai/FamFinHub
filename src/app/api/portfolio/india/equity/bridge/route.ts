...
import { accountsForHolder, type HolderKey } from "@/app/portfolio/india/equity/constants";

export const dynamic = "force-dynamic";

/** On-demand data for the India Equity performance page's "Why trust this number?" panel — mirrors
 * ../performance/page.tsx's own scope resolution (holder, price-only). */
export async function GET(req: NextRequest) {
  const sp = req.nextUrl.searchParams;
  const { ctx, channelAccounts } = await getIndiaPortfolioData();

  const hRaw = sp.get("h");
  const h: HolderKey = hRaw === "SANGEETH" || hRaw === "RIA" ? hRaw : "ALL";
  const accounts = h === "ALL" ? channelAccounts.EQUITY : (accountsForHolder(h) ?? channelAccounts.EQUITY);
